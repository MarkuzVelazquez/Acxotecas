...
if (trim($_POST['no_cuenta'])== '')
{
   ...
   $no_cuenta= mysql_real_escape_string($_POST['no_cuenta']);
...
//Licenciatura
if (trim($_POST['licenciatura'])== '')
{
       echo "<script type=\"text/javascript\">alert('Proporciona tu licenciatura');</script>";
}else{
  $licenciatura = mysql_real_escape_string($_POST['licenciatura']);
}
//Semestre
if (trim($_POST['semestre'])== '')
{
       echo "<script type=\"text/javascript\">alert('Proporciona tu semestre');</script>";
}else{
...
if (trim($_POST['turno'])== '')
{
       echo "<script type=\"text/javascript\">alert('Proporciona tu turno');</script>";
}else{
  $turno = mysql_real_escape_string($_POST['turno']);
}
//Grupo
if (trim($_POST['grupo'])== '')
{
       echo "<script type=\"text/javascript\">alert('Proporciona tu grupo');</script>";
}else{
...
echo "<script type=\"text/javascript\">alert('Este registro ya existe, verifica tu informacion');</script>";
 ...
 $query = "INSERT registros ( tipo , email , paterno , materno , nombre , dia , mes , anio , lu_na , edad , sexo , enfermedad , sangre , alergia , ocupacion , evento , categoria , fe_re , costo , equipo_uaem , no_cuenta, lic , sem , turno , grupo)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    
    $sentencia = $conexion->prepare( $query );
    
    $sentencia->bind_param( 'sssssisisissssssssisisiss' , $tipo , $email , $paterno  , $materno , $nombre , $dia  , $mes , $anio , $lugar , $edad , $sexo , $enfermedad , $sangre , $alergia , $ocupacion , $evento , $categoria , $fecha_reg , $costo , $taller, $cuenta , $licenciatura , $semestre , $turno , $grupo);
